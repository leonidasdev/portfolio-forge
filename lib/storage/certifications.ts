...
export async function uploadCertificationFile(
  ...
  const folderPath = certificationId ? `${userId}/${certificationId}` : `${userId}/temp`
  ...
  if (error) {
    logger.error('Certification upload failed', { error, filePath })
...
export async function deleteCertificationFile(filePath: string): Promise<void> {
  ...
  if (error) {
    logger.error('Certification delete failed', { error, filePath })
...
export async function moveToPermanentLocation(
  ...
  // Move file
  const { error } = await supabase.storage.from(BUCKET_NAME).move(tempFilePath, newPath)
