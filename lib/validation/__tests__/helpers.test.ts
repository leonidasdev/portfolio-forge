...
import { z } from 'zod'
import { validateBody, validateParams, validateQuery } from '../helpers'

// Type for ApiError with status property
interface ApiErrorWithStatus extends ApiError {
  status: number
}

// Create a minimal mock that satisfies NextRequest interface for testing
...
describe('validateBody', () => {
  ...
  it('should validate and return valid body data', async () => {
    ...
    const request = createMockRequest('http://localhost/api/test', body)
  ...
  it('should throw ApiError for missing required field', async () => {
    ...
    await expect(validateBody(request, testSchema)).rejects.toThrow(ApiError)
  ...
  it('should apply transformations', async () => {
    ...
    const result = await validateBody(request, schemaWithTransform)
...
describe('validateQuery', () => {
  ...
  it('should validate and return valid query params', () => {
    const request = createMockRequest('http://localhost/api/test?page=2&limit=20&sortBy=title')
  ...
  it('should parse boolean values correctly', () => {
    const request = createMockRequest('http://localhost/api/test?includeArchived=true')
  ...
  it('should throw ApiError for invalid query params', () => {
    const request = createMockRequest('http://localhost/api/test?page=-1') // min is 1

    expect(() => validateQuery(request, paginationSchema)).toThrow(ApiError)
  })

  it('should throw ApiError for value exceeding max', () => {
    const request = createMockRequest('http://localhost/api/test?limit=500') // max is 100
