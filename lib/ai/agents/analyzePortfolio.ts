...
export async function analyzePortfolio(userId: string): Promise<{
  ...
  const toneSignals = {
    ...
    strong:
      allText.match(/\b(achieved|delivered|improved|optimized|increased|reduced)\b/g)?.length || 0,
  }
  ...
  const senioritySignals = {
    ...
    senior:
      allText.match(/\b(senior|lead|principal|staff|architect|director|manager|head)\b/gi)
        ?.length || 0,
  }
...
Provide score, subscores, and 3-5 specific recommendations. Include suggestedRewrite for the top 2 issues.`
  ...
  try {
    ...
    const avgScore = Math.round(Object.values(subscores).reduce((sum, val) => sum + val, 0) / 6)
    ...
    if (!contentProfile.hasSummary) {
      recommendations.push({
        ...
        description:
          'A strong summary section is critical for making a first impression. Add 2-3 sentences highlighting your key strengths.',
      })
    }
    ...
    if (toneSignals.passive > toneSignals.active) {
      recommendations.push({
        ...
        description:
          'Replace passive constructions with active verbs (led, developed, created) to demonstrate ownership and impact.',
      })
    }
    ...
    if (contentProfile.skillsCount < 5) {
      recommendations.push({
        ...
        description:
          'Add more relevant technical skills to improve ATS matching and showcase your expertise.',
      })
    }
