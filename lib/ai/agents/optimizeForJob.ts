...
export async function optimizePortfolioForJob(
  ...
  if (
    seniorityText.includes('senior') ||
    seniorityText.includes('lead') ||
    seniorityText.includes('principal')
  ) {
    tone = 'senior'
  ...
  // Fetch user's portfolios using typed queries
  ...
  // Fetch all sections using typed queries
  ...
  const suggestedSkills = jobInsights.requiredSkills.filter(
    (reqSkill) =>
      !userSkills.some((userSkill) => userSkill.toLowerCase().includes(reqSkill.toLowerCase()))
  )
