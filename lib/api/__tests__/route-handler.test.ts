...
import { ApiError, apiResponse, withApiHandler } from '../route-handler'
...
const createMockResponse = (data: unknown, status: number = 200): MockResponse => ({
  status,
  ok: status < 400,
  ...
  describe('successful requests', () => {
    it('should return handler response on success', async () => {
      const handler = withApiHandler(async () => {
        return createMockResponse({ message: 'Success' }) as unknown as Response
      })

      const response = await handler(mockRequest as unknown as NextRequest)
    ...
    it('should pass request and context to handler', async () => {
      const mockHandler = jest.fn().mockResolvedValue(createMockResponse({ ok: true }))
      const handler = withApiHandler(mockHandler)
      const context = { params: { id: '123' } }

      await handler(mockRequest as unknown as NextRequest, context)
    ...
    it('should handle synchronous handlers', async () => {
      const handler = withApiHandler(() => {
        return createMockResponse({ sync: true }) as unknown as Response
      })
