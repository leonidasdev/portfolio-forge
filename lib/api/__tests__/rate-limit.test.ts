...
describe('rateLimitStore', () => {
  ...
  describe('check()', () => {
    it('should allow requests within rate limit', async () => {
      ...
      const result = await rateLimitStore.check(key, limit, windowSeconds)
    ...
    it('should track remaining requests correctly', async () => {
      ...
      const result1 = await rateLimitStore.check(key, limit, windowSeconds)
      expect(result1.remaining).toBe(2)

      const result2 = await rateLimitStore.check(key, limit, windowSeconds)
      expect(result2.remaining).toBe(1)

      const result3 = await rateLimitStore.check(key, limit, windowSeconds)
      expect(result3.remaining).toBe(0)

      // Fourth request should be denied
      const result4 = await rateLimitStore.check(key, limit, windowSeconds)
    ...
    it('should use sliding window algorithm', async () => {
      ...
      // Use up the limit
      await rateLimitStore.check(key, limit, windowSeconds)
      await rateLimitStore.check(key, limit, windowSeconds)
      ...
      return new Promise<void>((resolve) => {
        setTimeout(async () => {
          const afterExpiry = await rateLimitStore.check(key, limit, windowSeconds)
    ...
    it('should keep separate counts for different keys', async () => {
      const limit = 2
      const windowSeconds = 60

      // Use up limit for key1
      await rateLimitStore.check('key1', limit, windowSeconds)
      await rateLimitStore.check('key1', limit, windowSeconds)

      // key1 should be blocked
      expect((await rateLimitStore.check('key1', limit, windowSeconds)).allowed).toBe(false)

      // key2 should still work
      expect((await rateLimitStore.check('key2', limit, windowSeconds)).allowed).toBe(true)
    })
  })

  describe('peek()', () => {
    it('should return status without consuming a request', async () => {
      const key = 'peek-test'
      const limit = 3
      const windowSeconds = 60
      ...
      const peek1 = await rateLimitStore.peek(key, limit)
      expect(peek1.remaining).toBe(2)

      // Peek again should still show 2 remaining (not consumed)
      const peek2 = await rateLimitStore.peek(key, limit)
      expect(peek2.remaining).toBe(2)

      // Actual check should now consume
      const checkResult = await rateLimitStore.check(key, limit, windowSeconds)
  ...
  describe('reset()', () => {
    it('should clear rate limit for a specific key', async () => {
      ...
      await rateLimitStore.check(key, limit, windowSeconds)
      await rateLimitStore.check(key, limit, windowSeconds)
      expect((await rateLimitStore.check(key, limit, windowSeconds)).allowed).toBe(false)

      // Reset the key
      await rateLimitStore.reset(key)

      // Should be allowed again
      expect((await rateLimitStore.check(key, limit, windowSeconds)).allowed).toBe(true)
    })
  })
})
...
describe('checkRateLimitStatus', () => {
  ...
  it('should return status without consuming request', async () => {
    ...
    const status = await checkRateLimitStatus(request, config)
...
describe('resetRateLimit', () => {
  ...
  it('should reset rate limit for a key', async () => {
    ...
    const handler = jest.fn().mockImplementation(async () => NextResponse.json({}))
