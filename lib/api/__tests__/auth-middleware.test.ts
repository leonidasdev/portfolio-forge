...
import { AuthError, optionalAuth, requireAuth } from '../auth-middleware'
...
const createMockRequest = (url: string = 'http://localhost:3000/api/v1/test'): MockRequest => ({
  url,
  method: 'GET',
  headers: new Map(),
})
...
describe('Auth Middleware', () => {
  ...
  describe('requireAuth', () => {
    it('should return user and supabase client when authenticated', async () => {
      ...
      const result = await requireAuth(mockRequest as unknown as NextRequest)
    ...
    it('should throw AuthError when user is not authenticated', async () => {
      ...
      await expect(requireAuth(mockRequest as unknown as NextRequest)).rejects.toThrow(AuthError)
      await expect(requireAuth(mockRequest as unknown as NextRequest)).rejects.toThrow(
        'Unauthorized'
      )
    })
    ...
    it('should throw AuthError with 401 status on auth failure', async () => {
      ...
      try {
        await requireAuth(mockRequest as unknown as NextRequest)
  ...
  describe('optionalAuth', () => {
    it('should return user when authenticated', async () => {
      ...
      const result = await optionalAuth(mockRequest as unknown as NextRequest)
