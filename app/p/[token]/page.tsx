...
export default async function PublicPortfolioPage({ params }: { params: { token: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">This portfolio link is invalid or has been removed.</p>
        </div>
      </div>
    )
  }
  ...
  if (!portfolio.is_public) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Private Portfolio</h1>
          <p className="text-gray-600">This portfolio is not publicly accessible.</p>
        </div>
      </div>
    )
  }
  ...
  const visibleSections = await filterVisibleSections(sections || [], portfolio.user_id, supabase)
      ...
      {visibleSections.length === 0 ? (
        ...
        >
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: '#6b7280' }}>This portfolio has no visible content yet.</p>
          </div>
        </div>
      ) : (
