  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
        <SectionAddMenu portfolioId={portfolio.id} onSectionAdded={addSection} />
      </div>
      ...
      {sections.length > 0 && (
        <div className="space-y-4">
          ...
          <AIPortfolioAnalyzer
            ...
            onApplyRecommendation={(rec) => {
              ...
              if (section && rec.suggestedRewrite) {
                const existingContent = (section.custom_content as Record<string, unknown>) || {}
                const updatedContent =
                  section.section_type === 'about'
                    ? { text: rec.suggestedRewrite }
                    : { ...existingContent, description: rec.suggestedRewrite }

                updateSection({ ...section, custom_content: updatedContent })
              }
            }}
          />
          ...
          {/* Rewrite Portfolio */}
          <AIRewritePortfolio sections={sections} onSectionsUpdate={setSections} />

          {/* Job Optimizer */}
          <AIJobOptimizer sections={sections} onSectionsUpdate={setSections} />
        </div>
      )}
